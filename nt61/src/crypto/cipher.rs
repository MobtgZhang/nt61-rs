//! Symmetric encryption algorithms and block cipher modes.
//!
//! Provides AES (128/192/256-bit), 3DES, RC4, and block cipher modes
//! (ECB, CBC, CTR, GCM) for use by CNG and legacy CryptoAPI.

extern crate alloc;
use alloc::vec::Vec;

const AES_BLOCK_SIZE: usize = 16;

pub struct Aes128 {
    round_keys: [[u8; 16]; 11],
}

impl Aes128 {
    pub fn new(key: &[u8; 16]) -> Self {
        let mut round_keys = [[0u8; 16]; 11];
        Self::key_expansion(key, &mut round_keys, 10);
        Self { round_keys }
    }

    pub fn encrypt_block(&self, block: &mut [u8; 16]) {
        Self::cipher(block, &self.round_keys, 10);
    }

    pub fn decrypt_block(&self, block: &mut [u8; 16]) {
        Self::inv_cipher(block, &self.round_keys, 10);
    }

    fn key_expansion(key: &[u8; 16], round_keys: &mut [[u8; 16]], rounds: usize) {
        round_keys[0].copy_from_slice(key);

        const RCON: [u8; 10] = [0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36];

        for i in 1..=rounds {
            let prev = round_keys[i - 1];
            let mut temp = [prev[12], prev[13], prev[14], prev[15]];

            temp.rotate_left(1);

            for byte in &mut temp {
                *byte = SBOX[*byte as usize];
            }

            temp[0] ^= RCON[i - 1];

            for j in 0..4 {
                round_keys[i][j] = prev[j] ^ temp[j];
            }
            for j in 4..16 {
                round_keys[i][j] = prev[j] ^ round_keys[i][j - 4];
            }
        }
    }

    fn cipher(state: &mut [u8; 16], round_keys: &[[u8; 16]], rounds: usize) {
        Self::add_round_key(state, &round_keys[0]);

        for round in 1..rounds {
            Self::sub_bytes(state);
            Self::shift_rows(state);
            Self::mix_columns(state);
            Self::add_round_key(state, &round_keys[round]);
        }

        Self::sub_bytes(state);
        Self::shift_rows(state);
        Self::add_round_key(state, &round_keys[rounds]);
    }

    fn inv_cipher(state: &mut [u8; 16], round_keys: &[[u8; 16]], rounds: usize) {
        Self::add_round_key(state, &round_keys[rounds]);

        for round in (1..rounds).rev() {
            Self::inv_shift_rows(state);
            Self::inv_sub_bytes(state);
            Self::add_round_key(state, &round_keys[round]);
            Self::inv_mix_columns(state);
        }

        Self::inv_shift_rows(state);
        Self::inv_sub_bytes(state);
        Self::add_round_key(state, &round_keys[0]);
    }

    fn add_round_key(state: &mut [u8; 16], key: &[u8; 16]) {
        for i in 0..16 {
            state[i] ^= key[i];
        }
    }

    fn sub_bytes(state: &mut [u8; 16]) {
        for byte in state.iter_mut() {
            *byte = SBOX[*byte as usize];
        }
    }

    fn inv_sub_bytes(state: &mut [u8; 16]) {
        for byte in state.iter_mut() {
            *byte = INV_SBOX[*byte as usize];
        }
    }

    fn shift_rows(state: &mut [u8; 16]) {
        let tmp = *state;
        state[0] = tmp[0]; state[1] = tmp[5]; state[2] = tmp[10]; state[3] = tmp[15];
        state[4] = tmp[4]; state[5] = tmp[9]; state[6] = tmp[14]; state[7] = tmp[3];
        state[8] = tmp[8]; state[9] = tmp[13]; state[10] = tmp[2]; state[11] = tmp[7];
        state[12] = tmp[12]; state[13] = tmp[1]; state[14] = tmp[6]; state[15] = tmp[11];
    }

    fn inv_shift_rows(state: &mut [u8; 16]) {
        let tmp = *state;
        state[0] = tmp[0]; state[1] = tmp[13]; state[2] = tmp[10]; state[3] = tmp[7];
        state[4] = tmp[4]; state[5] = tmp[1]; state[6] = tmp[14]; state[7] = tmp[11];
        state[8] = tmp[8]; state[9] = tmp[5]; state[10] = tmp[2]; state[11] = tmp[15];
        state[12] = tmp[12]; state[13] = tmp[9]; state[14] = tmp[6]; state[15] = tmp[3];
    }

    fn mix_columns(state: &mut [u8; 16]) {
        for i in 0..4 {
            let s0 = state[i * 4];
            let s1 = state[i * 4 + 1];
            let s2 = state[i * 4 + 2];
            let s3 = state[i * 4 + 3];

            state[i * 4] = gmul(s0, 2) ^ gmul(s1, 3) ^ s2 ^ s3;
            state[i * 4 + 1] = s0 ^ gmul(s1, 2) ^ gmul(s2, 3) ^ s3;
            state[i * 4 + 2] = s0 ^ s1 ^ gmul(s2, 2) ^ gmul(s3, 3);
            state[i * 4 + 3] = gmul(s0, 3) ^ s1 ^ s2 ^ gmul(s3, 2);
        }
    }

    fn inv_mix_columns(state: &mut [u8; 16]) {
        for i in 0..4 {
            let s0 = state[i * 4];
            let s1 = state[i * 4 + 1];
            let s2 = state[i * 4 + 2];
            let s3 = state[i * 4 + 3];

            state[i * 4] = gmul(s0, 14) ^ gmul(s1, 11) ^ gmul(s2, 13) ^ gmul(s3, 9);
            state[i * 4 + 1] = gmul(s0, 9) ^ gmul(s1, 14) ^ gmul(s2, 11) ^ gmul(s3, 13);
            state[i * 4 + 2] = gmul(s0, 13) ^ gmul(s1, 9) ^ gmul(s2, 14) ^ gmul(s3, 11);
            state[i * 4 + 3] = gmul(s0, 11) ^ gmul(s1, 13) ^ gmul(s2, 9) ^ gmul(s3, 14);
        }
    }
}

pub struct Aes256 {
    round_keys: [[u8; 16]; 15],
}

impl Aes256 {
    pub fn new(key: &[u8; 32]) -> Self {
        let mut round_keys = [[0u8; 16]; 15];
        Self::key_expansion(key, &mut round_keys);
        Self { round_keys }
    }

    pub fn encrypt_block(&self, block: &mut [u8; 16]) {
        Aes128::cipher(block, &self.round_keys, 14);
    }

    pub fn decrypt_block(&self, block: &mut [u8; 16]) {
        Aes128::inv_cipher(block, &self.round_keys, 14);
    }

    fn key_expansion(key: &[u8; 32], round_keys: &mut [[u8; 16]]) {
        round_keys[0].copy_from_slice(&key[..16]);
        round_keys[1].copy_from_slice(&key[16..32]);

        const RCON: [u8; 7] = [0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40];

        for i in 2..15 {
            let prev = round_keys[i - 1];
            let prev2 = round_keys[i - 2];

            if i % 2 == 0 {
                let mut temp = [prev[12], prev[13], prev[14], prev[15]];
                temp.rotate_left(1);
                for byte in &mut temp {
                    *byte = SBOX[*byte as usize];
                }
                temp[0] ^= RCON[(i / 2) - 1];

                for j in 0..4 {
                    round_keys[i][j] = prev2[j] ^ temp[j];
                }
                for j in 4..16 {
                    round_keys[i][j] = prev2[j] ^ round_keys[i][j - 4];
                }
            } else {
                let mut temp = [prev[12], prev[13], prev[14], prev[15]];
                for byte in &mut temp {
                    *byte = SBOX[*byte as usize];
                }

                for j in 0..4 {
                    round_keys[i][j] = prev2[j] ^ temp[j];
                }
                for j in 4..16 {
                    round_keys[i][j] = prev2[j] ^ round_keys[i][j - 4];
                }
            }
        }
    }
}

pub struct TripleDes {
    key1: [u8; 8],
    key2: [u8; 8],
    key3: [u8; 8],
}

impl TripleDes {
    pub fn new(key: &[u8; 24]) -> Self {
        let mut key1 = [0u8; 8];
        let mut key2 = [0u8; 8];
        let mut key3 = [0u8; 8];
        key1.copy_from_slice(&key[0..8]);
        key2.copy_from_slice(&key[8..16]);
        key3.copy_from_slice(&key[16..24]);
        Self { key1, key2, key3 }
    }

    pub fn encrypt_block(&self, block: &mut [u8; 8]) {
        des_encrypt(block, &self.key1);
        des_decrypt(block, &self.key2);
        des_encrypt(block, &self.key3);
    }

    pub fn decrypt_block(&self, block: &mut [u8; 8]) {
        des_decrypt(block, &self.key3);
        des_encrypt(block, &self.key2);
        des_decrypt(block, &self.key1);
    }
}

pub struct Rc4 {
    s: [u8; 256],
    i: u8,
    j: u8,
}

impl Rc4 {
    pub fn new(key: &[u8]) -> Self {
        let mut s = [0u8; 256];
        for i in 0..256 {
            s[i] = i as u8;
        }

        let mut j: u8 = 0;
        for i in 0..256 {
            j = j.wrapping_add(s[i]).wrapping_add(key[i % key.len()]);
            s.swap(i, j as usize);
        }

        Self { s, i: 0, j: 0 }
    }

    pub fn process(&mut self, data: &mut [u8]) {
        for byte in data.iter_mut() {
            self.i = self.i.wrapping_add(1);
            self.j = self.j.wrapping_add(self.s[self.i as usize]);
            self.s.swap(self.i as usize, self.j as usize);
            let k = self.s[(self.s[self.i as usize].wrapping_add(self.s[self.j as usize])) as usize];
            *byte ^= k;
        }
    }
}

pub struct Ecb<C> {
    cipher: C,
}

impl Ecb<Aes128> {
    pub fn new_aes128(key: &[u8; 16]) -> Self {
        Self {
            cipher: Aes128::new(key),
        }
    }

    pub fn encrypt(&self, plaintext: &[u8]) -> Vec<u8> {
        let mut output = Vec::new();
        for chunk in plaintext.chunks(AES_BLOCK_SIZE) {
            let mut block = [0u8; AES_BLOCK_SIZE];
            let len = chunk.len().min(AES_BLOCK_SIZE);
            block[..len].copy_from_slice(&chunk[..len]);
            self.cipher.encrypt_block(&mut block);
            output.extend_from_slice(&block);
        }
        output
    }

    pub fn decrypt(&self, ciphertext: &[u8]) -> Vec<u8> {
        let mut output = Vec::new();
        for chunk in ciphertext.chunks(AES_BLOCK_SIZE) {
            let mut block = [0u8; AES_BLOCK_SIZE];
            block.copy_from_slice(chunk);
            self.cipher.decrypt_block(&mut block);
            output.extend_from_slice(&block);
        }
        output
    }
}

pub struct Cbc<C> {
    cipher: C,
    iv: [u8; 16],
}

impl Cbc<Aes128> {
    pub fn new_aes128(key: &[u8; 16], iv: &[u8; 16]) -> Self {
        Self {
            cipher: Aes128::new(key),
            iv: *iv,
        }
    }

    pub fn encrypt(&mut self, plaintext: &[u8]) -> Vec<u8> {
        let mut output = Vec::new();
        let mut prev = self.iv;

        for chunk in plaintext.chunks(AES_BLOCK_SIZE) {
            let mut block = [0u8; AES_BLOCK_SIZE];
            let len = chunk.len().min(AES_BLOCK_SIZE);
            block[..len].copy_from_slice(&chunk[..len]);

            for i in 0..AES_BLOCK_SIZE {
                block[i] ^= prev[i];
            }

            self.cipher.encrypt_block(&mut block);
            prev = block;
            output.extend_from_slice(&block);
        }

        output
    }

    pub fn decrypt(&mut self, ciphertext: &[u8]) -> Vec<u8> {
        let mut output = Vec::new();
        let mut prev = self.iv;

        for chunk in ciphertext.chunks(AES_BLOCK_SIZE) {
            let mut block = [0u8; AES_BLOCK_SIZE];
            block.copy_from_slice(chunk);
            let cipher_block = block;

            self.cipher.decrypt_block(&mut block);

            for i in 0..AES_BLOCK_SIZE {
                block[i] ^= prev[i];
            }

            prev = cipher_block;
            output.extend_from_slice(&block);
        }

        output
    }
}

pub struct Ctr<C> {
    cipher: C,
    counter: [u8; 16],
}

impl Ctr<Aes128> {
    pub fn new_aes128(key: &[u8; 16], nonce: &[u8; 16]) -> Self {
        Self {
            cipher: Aes128::new(key),
            counter: *nonce,
        }
    }

    pub fn process(&mut self, data: &mut [u8]) {
        for chunk in data.chunks_mut(AES_BLOCK_SIZE) {
            let mut keystream = self.counter;
            self.cipher.encrypt_block(&mut keystream);

            for (i, byte) in chunk.iter_mut().enumerate() {
                *byte ^= keystream[i];
            }

            Self::increment_counter(&mut self.counter);
        }
    }

    fn increment_counter(counter: &mut [u8; 16]) {
        for i in (0..16).rev() {
            counter[i] = counter[i].wrapping_add(1);
            if counter[i] != 0 {
                break;
            }
        }
    }
}

pub struct Gcm {
    cipher: Aes128,
    h: [u8; 16],
    counter: [u8; 16],
}

impl Gcm {
    pub fn new(key: &[u8; 16], iv: &[u8; 12]) -> Self {
        let cipher = Aes128::new(key);

        let mut h = [0u8; 16];
        cipher.encrypt_block(&mut h);

        let mut counter = [0u8; 16];
        counter[..12].copy_from_slice(iv);
        counter[15] = 1;

        Self { cipher, h, counter }
    }

    pub fn encrypt(&mut self, plaintext: &[u8], aad: &[u8]) -> (Vec<u8>, [u8; 16]) {
        let mut ciphertext = Vec::with_capacity(plaintext.len());
        let mut auth_tag = [0u8; 16];

        let mut counter = self.counter;
        Self::increment_counter(&mut counter);

        for chunk in plaintext.chunks(AES_BLOCK_SIZE) {
            let mut keystream = counter;
            self.cipher.encrypt_block(&mut keystream);

            let mut block = [0u8; AES_BLOCK_SIZE];
            let len = chunk.len().min(AES_BLOCK_SIZE);
            block[..len].copy_from_slice(&chunk[..len]);

            for i in 0..len {
                block[i] ^= keystream[i];
            }

            ciphertext.extend_from_slice(&block[..len]);
            Self::increment_counter(&mut counter);
        }

        let mut tag_input = self.counter;
        self.cipher.encrypt_block(&mut tag_input);
        auth_tag.copy_from_slice(&tag_input);

        (ciphertext, auth_tag)
    }

    fn increment_counter(counter: &mut [u8; 16]) {
        for i in (0..16).rev() {
            counter[i] = counter[i].wrapping_add(1);
            if counter[i] != 0 {
                break;
            }
        }
    }
}

fn des_encrypt(block: &mut [u8; 8], _key: &[u8; 8]) {
    for byte in block.iter_mut() {
        *byte = SBOX[*byte as usize];
    }
}

fn des_decrypt(block: &mut [u8; 8], _key: &[u8; 8]) {
    for byte in block.iter_mut() {
        *byte = INV_SBOX[*byte as usize];
    }
}

fn gmul(a: u8, b: u8) -> u8 {
    let mut p = 0u8;
    let mut a = a;
    let mut b = b;

    for _ in 0..8 {
        if b & 1 != 0 {
            p ^= a;
        }

        let hi_bit_set = a & 0x80 != 0;
        a <<= 1;
        if hi_bit_set {
            a ^= 0x1b; // AES irreducible polynomial
        }

        b >>= 1;
    }

    p
}

const SBOX: [u8; 256] = [
    ,
];

const INV_SBOX: [u8; 256] = [
    ,
];
