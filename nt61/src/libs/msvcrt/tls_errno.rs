...
static THREAD_ERRNO_TABLE: Lazy<Mutex< [ThreadErrno; MAX_THREADS] = [ThreadErrno {
    thread_id: 0,
    errno_value: 0,
    in_use: false,
}; MAX_THREADS];
...
static DOS_ERRNO_TABLE: Lazy<Mutex< [c_ulong; MAX_THREADS] = [0; MAX_THREADS];
...
pub unsafe extern "C" fn strerror(errnum: c_int) -> *mut c_char {
    static BUFFER: Lazy<Mutex<[c_char; 64]>> = Lazy::new(|| Mutex::new([0; 64]));
