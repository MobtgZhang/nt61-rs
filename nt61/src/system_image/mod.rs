...
pub fn build_cmd_exe_for_machine(machine: u16) -> core::mem::ManuallyDrop<Vec<u8>> {
    ...
    if machine == MACHINE_X86_64 {
        ...
        unsafe {
            ...
            // used once and consumed within this call. Wrap
            // the returned Vec in ManuallyDrop so the kernel
            // heap never sees a dealloc() for a static pointer
            // (which would otherwise corrupt KernelHeap on the
            // next allocation).
            let ptr = STATIC_BUF.as_mut_ptr();
            let v = alloc::vec::Vec::from_raw_parts(ptr, len, 8192);
            return core::mem::ManuallyDrop::new(v);
        }
    }
    // Other arches: fall back to the on-demand builder. This is
    // the path `dump_cmd` and the host-side tests use. Wrap the
    // resulting Vec in ManuallyDrop so the static-buffer path and
    // the heap-buffer path share a single safe return type.
    core::mem::ManuallyDrop::new(build_cmd_exe(machine))
}
...
/// Allocate a Vec<u8> with the given size, using the kernel allocator.
/// This is a safety wrapper around Vec::with_capacity to provide better
/// error handling and avoid potential kernel-mode pitfalls.
pub fn safe_alloc_vec(size: usize) -> Option<alloc::vec::Vec<u8>> {
    if size == 0 {
        return Some(alloc::vec::Vec::new());
    }
    let mut v = alloc::vec::Vec::with_capacity(size);
    if v.capacity() < size {
        return None;
    }
    // SAFETY: capacity is at least `size`, so we can set the length.
