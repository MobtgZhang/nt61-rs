//! riscv64 per-CPU implementation.
//
//! Uses the `tp` (thread pointer) register, which is typically set by
//! the bootloader to point to the per-CPU area. We use `sscratch` CSR
//! as a swap register: on entry from user mode, we swap tp and sscratch
//! to get the kernel's per-CPU area, and swap back on return.
//
...
/// Initialize the per-CPU area for `cpu_id` and install it as the `tp`
/// register. For BSP (cpu_id=0) uses the static `PER_CPU_0`; for APs
...
pub unsafe extern "Rust" fn __percpu_init(cpu_id: u32) -> u64 {
    ...
    if page == 0 {
        crate::mm::fatal_alloc::<PerCpuArea>("riscv64 percpu init");
    }
    ...
    // Install as tp for this CPU (write to tp CSR)
    set_tp(page);
...
/// Set the `tp` CSR to the given value.
#[inline(always)]
fn set_tp(val: u64) {
    unsafe {
...
/// Return the cached per-CPU base for `hart_id`, allocating one if
/// necessary.
///
/// Phase 1 implementation: BSP returns `PER_CPU_0`; secondary
/// harts call [`allocate`]. The secondary trampoline
/// (`__smp_secondary_entry`) calls this to install a per-hart
/// per-CPU area before waiting for the first IPI.
pub fn per_cpu_base_for(hart_id: u32) -> u64 {
    if hart_id == 0 {
        return core::ptr::addr_of_mut!(PER_CPU_0) as u64;
    }
    let cached = PER_CPU_PAGES[hart_id as usize].load(Ordering::Acquire);
    if cached != 0 {
        return cached;
    }
    unsafe { allocate(hart_id) }
}
...
/// Reads `tp` to find the per-CPU base.
#[no_mangle]
